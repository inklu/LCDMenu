...
void setup() {
  ...
  Serial.println((unsigned long)&mbc.newButtonOk(A4,139),HEX);
  Serial.println((unsigned long)&mbc.newButtonUp(A4,42),HEX);
  Serial.println((unsigned long)&mbc.newButtonDown(A4,93),HEX);
  Serial.println((unsigned long)&mbc.newButtonBack(A4,14),HEX);
  //m.create();
  //mbc.bindMenu(m);
...
void loop() {
  ...
  mbc.run(); //длительное нажатие на OK автивирует меню, длительное нажатие на BACK деактивирует меню
